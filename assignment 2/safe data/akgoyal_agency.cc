...
//struct Team * dummyHeadTeam = createTeam(-1, "DUMMY", "DUMMY");
//struct Team * teamsRootNode = dummyHeadTeam;
struct Team * teamsRootNode = createTeam(-1, "DUMMY", "DUMMY");

bool sortAllPlayersOfATeam(string teamName, SearchCriteria criteria){
	struct Team * teamCounter = teamsRootNode;
...
bool addTeamToList(Team * addThisTeam){
	...
	struct Team * teamCounter = teamsRootNode;
	//struct Team * teamCounter = (struct Team*) malloc(sizeof(struct Team));
	//teamCounter = &teamsRootNode;
	while(teamCounter->next != NULL){
...
bool addPlayerToTeam(Team * teamNode, PlayerNode * newPlayer){
	struct PlayerNode * playerNodeCounter = teamNode->linkedPlayerFirstNode;
...
bool menuOne(){
	...
	if(validation==true){
		...
		struct Team * newTeam = createTeam(teamId, uniqueName, city);
...
bool menuTwo(){
	...
	if(validation == true){
		...
		struct Player * newPlayer = createPlayer(id, teamId, name, age, phone, position, salary);
		...
		if(addPlayerToArrayIndex != -1){
			...
			while(teamCounter->next != NULL){
				if(teamCounter->next->id == teamId){
					struct PlayerNode * newPlayerNode = createPlayerNode(id, addPlayerToArrayIndex);
...
void runProgram(){
	...
	showMenu();
	cin>>optionSelect;
	while(optionSelect<1 || optionSelect>14){
...
bool createAndRunTestCases(){
	...
	//Create two teams
	struct Team * newTeam1 = createTeam(1, "USA", "NYC");
	bool addTeamToListAnswer1 = addTeamToList(newTeam1);
	struct Team * newTeam2 = createTeam(2, "CANADA", "TORONTO");
	bool addTeamToListAnswer2 = addTeamToList(newTeam2);
	
	//create 5 players
	struct Player * newPlayer1 = createPlayer(1, 1, "Mark", 23, "123456", "Forward", 1000.00);
	struct Player * newPlayer2 = createPlayer(2, 1, "Jim", 34, "123676", "Goalkeeper", 9000.00);
	struct Player * newPlayer3 = createPlayer(3, 1, "Adam", 22, "677777", "Middle", 6000.00);
	struct Player * newPlayer4 = createPlayer(4, 2, "Freddy", 21, "999999", "Forward", 500.00);
	struct Player * newPlayer5 = createPlayer(5, 2, "Brad", 20, "8888888", "Goalkeeper", 600.00);
	...
	if(addPlayerToArrayIndex1 != -1){
		...
		while(teamCounter->next != NULL){
			if(teamCounter->next->id == 1){
				struct PlayerNode * newPlayerNode = createPlayerNode(1, addPlayerToArrayIndex1);
				bool addPlayerToTeamAnswer = addPlayerToTeam(teamCounter->next, newPlayerNode);
				break;
			}
			teamCounter = teamCounter->next;
	...
	if(addPlayerToArrayIndex2 != -1){
		...
		while(teamCounter->next != NULL){
			if(teamCounter->next->id == 1){
				struct PlayerNode * newPlayerNode = createPlayerNode(2, addPlayerToArrayIndex2);
				bool addPlayerToTeamAnswer = addPlayerToTeam(teamCounter->next, newPlayerNode);
				break;
			}
			teamCounter = teamCounter->next;
	...
	if(addPlayerToArrayIndex3 != -1){
		...
		while(teamCounter->next != NULL){
			if(teamCounter->next->id == 1){
				struct PlayerNode * newPlayerNode = createPlayerNode(3, addPlayerToArrayIndex3);
				bool addPlayerToTeamAnswer = addPlayerToTeam(teamCounter->next, newPlayerNode);
				break;
			}
			teamCounter = teamCounter->next;
	...
	if(addPlayerToArrayIndex4 != -1){
		...
		while(teamCounter->next != NULL){
			if(teamCounter->next->id == 2){
				struct PlayerNode * newPlayerNode = createPlayerNode(4, addPlayerToArrayIndex4);
				bool addPlayerToTeamAnswer = addPlayerToTeam(teamCounter->next, newPlayerNode);
				break;
			}
			teamCounter = teamCounter->next;
	...
	if(addPlayerToArrayIndex5 != -1){
		...
		while(teamCounter->next != NULL){
			if(teamCounter->next->id == 2){
				struct PlayerNode * newPlayerNode = createPlayerNode(5, addPlayerToArrayIndex5);
				bool addPlayerToTeamAnswer = addPlayerToTeam(teamCounter->next, newPlayerNode);
				break;
			}
			teamCounter = teamCounter->next;
...
int main ()
{
	...
	struct Team * teamCounterForDelete = teamsRootNode;
